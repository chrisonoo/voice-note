...
def cleanup_old_temp_files(max_age_hours=24):
    ...
    # Znajdź wszystkie pliki WAV w katalogu tymczasowym
    wav_pattern = os.path.join(config.AUDIO_TMP_DIR, "*.wav")
    for wav_file in glob.glob(wav_pattern):
        try:
            ...
            if os.path.getmtime(wav_file) < cutoff_time:
                os.remove(wav_file)
                cleaned_count += 1
                print(f"Usunięto stary plik tymczasowy: {os.path.basename(wav_file)}")
        except OSError as e:
            print(f"Błąd podczas usuwania {wav_file}: {e}")
